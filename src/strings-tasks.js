...
function countVowels(str) {
  ...
  return str.split('').filter((cur) => vowels.includes(cur)).length;
}
...
function isPalindrome(str) {
  const len = str.length % 2 === 0 ? str.length / 2 : (str.length - 1) / 2;
  ...
  for (let i = 0; i < len; i += 1) {
    if (strToLow[i] !== strToLow.at(-1 - i)) return false;
  }
  return true;
}
 ...
 * the function returns the0 first one encountered.
 *
...
function findLongestWord(sentence) {
  const getMaxWord = (a, b) => (a.length >= b.length ? a : b);
  return sentence.split(' ').reduce((ac, cur) => getMaxWord(ac, cur), '');
}
  ...
  return str
    ...
    .map((el) =>
      el
        .split('')
        .map((symb) =>
          /[A-Z]/.test(symb) ? symb.toLowerCase() : symb.toUpperCase()
        )
        .join('')
    )
    .join(' ');
